...
Before starting each step, verify the `GATE` condition. Each gate checks that the prior step's required artifact exists. After completing each step, confirm the `Checkpoint` artifact is present.
